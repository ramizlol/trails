...
# Source DEM itself remains ~30 m resolution.
...
def get_route_profile(
    ...
    if target_distance_miles < 4.0:
        ...
        return {
            ...
            # Number of partial routes retained
            # at each search depth.
            "beam_width": 1000,

            # Simplified OSM graph means one edge
            # can represent quite a bit of real trail.
            "beam_max_steps": 110,

            # Candidate can only exceed target by this
            # much before being discarded.
            "candidate_max_ratio": 1.12
        }
...
def graph_debug_segments(G):
    """
    Return all trail segments currently allowed
    by the routing graph.

    We collapse the usual two directed versions
    of the same physical trail segment.
    """
...
def route_score(
    ...
    # Short routes can reasonably contain
    # shared trailhead stems.
    if (
        target_distance_meters
        <
        4
...
def beam_search_short_loop(
    ...
    """
    TRUE closed-loop beam search.

    Important difference:

    We DO NOT append a shortest path home.

    The beam explores one actual trail edge
    at a time.

    A route becomes a candidate ONLY when
    the search itself reaches start_node.
    """
    ...
    # --------------------------------------------------------
    # DISTANCE-TO-HOME LOWER BOUND
    # --------------------------------------------------------
    #
    # We still calculate shortest distance back to start,
    # but ONLY to decide whether a partial route could
    # possibly finish within the requested mileage.
    #
    # We never attach this path to the candidate.
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                "Could not calculate return-distance "
                "lower bounds: "
                +
                str(exc)
            )
    ...
    # Best closed route even if outside limits.
    best_closed_route = None
    best_closed_metrics = None
    best_closed_score = float(
        "inf"
    )
    ...
    for depth in range(
        ...
        for state in beam:
            ...
            for neighbor in S.successors(
                ...
                # Distance and cumulative ascent cannot
                # decrease later, so these are safe prunes.
                if (
                    new_distance
                    >
                    max_acceptable_distance
                ...
                if (
                    ...
                    # Don't treat tiny accidental circles
                    # at the trailhead as real candidates.
                    if (
                        new_distance
                        <
                        target_distance_meters
                    ...
                    # Once we hit the start again,
                    # stop this state here.
                    #
                    # We do not run through the
                    # trailhead and start another loop.
                    continue


                # -----------------------------------------
                # MUST STILL BE ABLE TO RETURN HOME
                # -----------------------------------------
                ...
                distance_home_lower_bound = float(
                    return_distance[
                        neighbor
                    ]
                )
                ...
                minimum_possible_final_distance = (
                    new_distance
                    +
                    distance_home_lower_bound
                )


                # Even the shortest possible way home
                # would exceed maximum acceptable distance.
                if (
                    minimum_possible_final_distance
                    >
                    max_acceptable_distance
                ...
                # -----------------------------------------
                # PARTIAL ROUTE PRIORITY
                # -----------------------------------------

                # How close the shortest theoretically
                # closable version of this state is to
                # the target mileage.
                ...
                # Elevation accumulated per meter so far.
                if new_distance > 0:
                ...
                # Compare percent of target gain accumulated
                # with percent of mileage accumulated.
                ...
                # Re-visiting a junction is allowed.
                # It may be necessary for a lollipop route.
                node_revisited = (
                    neighbor
                    in
                    route
                ...
                # Too much gain too early is particularly
                # bad for a 200-ft target.
                gain_overshoot = max(
                    0.0,
                    new_gain
                    -
        ...
        for (
            ...
            # Fine buckets intentionally preserve
            # multiple ways of arriving at the same node.
            distance_bucket = int(
                state["distance"]
                /
                25.0
    ...
    if best_closed_route is not None:
        ...
        raise HTTPException(
            ...
            detail=(
                "True closed-loop beam search did not "
                "find a route inside the quality limits. "
                "Best closed route found was "
                +
                str(
                    round(
                        best_distance,
                ...
                +
                " ft gain. Check the gray Allowed Trail "
                "Network layer to confirm the trails you "
                "would use manually are available."
            )
        )
    ...
    raise HTTPException(
        ...
        detail=(
            "The beam search never found a closed loop "
            "inside the allowed distance/gain search "
            "space. Check the Allowed Trail Network layer."
        )
    )


# ============================================================
# WAYPOINT PATH FOR LONGER ROUTES
# ============================================================
...
# ============================================================
# DIAGNOSTIC TRAIL NETWORK ENDPOINT
# ============================================================
...
def generate_route(
    ...
    try:
        ...
        snapped_start_lat = float(
            G.nodes[start_node]["y"]
        )

        snapped_start_lon = float(
            G.nodes[start_node]["x"]
        )
        ...
        if same_point:
            ...
            if (
                ...
                route_type = (
                    "true closed-loop trail beam search"
                )
...
showNetworkCheckbox.addEventListener(
    "change",
...
async function loadTrailNetwork(
    data
) {
    ...
    if (!text) {
        ...
        throw new Error(
            "Trail-network endpoint returned no data."
        );
    }
    ...
    try {
        ...
        throw new Error(
            "Invalid trail-network response."
        );
    }
...
async function generateRoute() {
    ...
    results.innerHTML =
        ...
        "Loading the allowed trail network..." +
    ...
    try {
        ...
        // ------------------------------------------------
        // FIRST:
        // Draw exactly what the routing engine can use.
        //
        // This remains visible even if route generation
        // fails.
        // ------------------------------------------------

        await loadTrailNetwork(
            data
        );
        ...
        results.innerHTML =
            ...
            "Trail network loaded. Searching actual " +
            "closed-loop segment combinations..." +
        ...
        // Generate the route.
        //
        // The graph should now normally be cached.
        // ------------------------------------------------
        ...
        if (!text) {
            ...
            throw new Error(
                "Server returned an empty route response."
            );
        }
        ...
        // IMPORTANT:
        //
        // We intentionally DO NOT remove the gray
        // allowed trail network here.
        //
        // If routing fails, you can compare your
        // manually imagined route with the exact
        // trails our algorithm knows about.
        ...
        results.innerHTML =
            ...
            "The gray lines are the trails currently " +
            "available to the routing algorithm. If your " +
            "manual route uses a trail that is not gray, " +
            "we have a trail-data/filter problem. If all " +
            "of your manual route is gray, the remaining " +
            "problem is the search algorithm." +
